...
export const useAutoImport = ({ fileMap, toKey, toValue }) => {
  ...
  // 获取结果
  const toResult = ({ fileMap, toKeyCallback, toValueCallback } = { fileMap: {}, toKeyCallback: toKey, toValueCallback: toValue }) => {
    const result: AutoImportResult = { modules: {} };
  ...
  const result = toResult({ fileMap, toKeyCallback: toKey, toValueCallback: toValue });
